...
from models.feat_enhancer_lite import FeatEnhancerLite
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            s = 256  # 2x min stride
            pred, _ = self.forward(torch.zeros(1, ch, s, s))
        ...
        if isinstance(m, IAuxDetect):
            s = 256  # 2x min stride
            pred, _ = self.forward(torch.zeros(1, ch, s, s))
            m.stride = torch.tensor([s / x.shape[-2] for x in pred[:4]])  # forward
            # print(m.stride)
            check_anchor_order(m)
            m.anchors /= m.stride.view(-1, 1, 1)
            self.stride = m.stride
    ...
    def forward(self, x, augment=False, profile=False):
        if augment:
            ...
            for si, fi in zip(s, f):
                ...
                yi, _ = self.forward_once(xi, profile)
                yi = yi[0]
                yi[..., :4] /= si
                if fi == 2:
            ...
            pred, loss_feat = self.forward_once(x, profile)
            if self.training:
                return pred, loss_feat
            else:
                return pred
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.use_feat_enhancer and self.feat_enhancer is not None and m.i in self.feat_layer_ids:
                ...
                if m.i == self.feat_layer_ids[-1]:
                    p3 = ms_feats[self.feat_layer_ids[0]]
                    p4 = ms_feats[self.feat_layer_ids[1]]
                    p5 = ms_feats[self.feat_layer_ids[2]]

                    p3, p4, p5, feat_loss = self.feat_enhancer(p3, p4, p5)

                    y[self.feat_layer_ids[0]] = p3
                    y[self.feat_layer_ids[1]] = p4
                    x = p5
